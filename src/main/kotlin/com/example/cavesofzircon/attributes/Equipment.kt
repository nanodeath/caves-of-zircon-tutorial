...
class Equipment(initialWeapon: GameEntity<Weapon>, initialArmor: GameEntity<Armor>) : BaseAttribute(), DisplayableAttribute {
    ...
    val attackValue: Int get() = weaponProperty.value.attackValue + armorProperty.value.attackValue

    val defenseValue: Int get() = weaponProperty.value.defenseValue + armorProperty.value.defenseValue

    val armorName: String get() = armorProperty.value.name

    val weaponName: String get() = weaponProperty.value.name
    ...
    private val weaponStats: String get() = " A: ${weapon.attackValue} D: ${weapon.defenseValue}"

    private val armorStats: String get() = " A: ${armor.attackValue} D: ${armor.defenseValue}"

    fun equip(inventory: Inventory, combatItem: GameCombatItem): GameCombatItem {
        combatItem.takeIfType<Weapon>()?.let { return equipWeapon(inventory, it) }
        combatItem.takeIfType<Armor>()?.let { return equipArmor(inventory, it) }
